#!/usr/bin/env python3
"""
Complete RoBERTa Multi-label Issue Classifier (Full Training)
Same treatment as DistilBERT for fair comparison
"""
...
class RobertaIssueDataset(Dataset):
    ...
    def __init__(self, texts: List[str], labels: List[List[str]], tokenizer, max_length: int = 512):
        ...
        # Same label setup as DistilBERT
...
class RobertaTrainer:
    """Complete RoBERTa trainer (same as DistilBERT)"""
    ...
    def prepare_datasets(self, train_df: pd.DataFrame, test_df: pd.DataFrame, max_length: int = 512):
        ...
        self.train_dataset = RobertaIssueDataset(
        ...
        self.val_dataset = RobertaIssueDataset(
    ...
    def train_model(self, epochs: int = 3, batch_size: int = 4, learning_rate: float = 2e-5):
        ...
        # Initialize model
        self.model = RobertaMultiLabelClassifier(self.num_labels)
        ...
        model_path = f"models/roberta_classifier_{timestamp}"
    ...
    def plot_training_curves(self, train_losses: List[float], val_losses: List[float], timestamp: str):
        ...
        plt.title('RoBERTa Training Progress', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig(f'results/roberta_training_curves_{timestamp}.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        logger.info(f"📈 Training curves saved to: results/roberta_training_curves_{timestamp}.png")
    ...
    def evaluate_model(self, threshold: float = 0.5) -> Dict:
        ...
        # Print results
        print(f"\n🎯 RoBERTa Evaluation Results (threshold={threshold}):")
        ...
        # Save results
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        results_file = f"results/roberta_evaluation_{timestamp}.json"
...
def compare_with_distilbert(roberta_results):
    """Compare RoBERTa results with DistilBERT"""
    
    print("\n🔄 RoBERTa vs DistilBERT Comparison")
    print("=" * 50)
    
    # Load DistilBERT results
    results_files = [f for f in os.listdir('results') if f.startswith('distilbert_evaluation_')]
    if results_files:
        with open(f"results/{sorted(results_files)[-1]}", 'r') as f:
            distilbert_results = json.load(f)
        
        distilbert_micro = distilbert_results['metrics']['micro_f1']
        distilbert_macro = distilbert_results['metrics']['macro_f1']
        
        print(f"📊 Overall Performance:")
        print(f"                    DistilBERT    RoBERTa    Difference")
        print(f"  Micro F1:         {distilbert_micro:.3f}        {roberta_results['micro_f1']:.3f}     {roberta_results['micro_f1']-distilbert_micro:+.3f}")
        print(f"  Macro F1:         {distilbert_macro:.3f}        {roberta_results['macro_f1']:.3f}     {roberta_results['macro_f1']-distilbert_macro:+.3f}")
        
        print(f"\n📊 Per-label F1 Comparison:")
        for label in roberta_results['per_label_f1']:
            dist_f1 = distilbert_results['metrics']['per_label_f1'][label]
            rob_f1 = roberta_results['per_label_f1'][label]
            diff = rob_f1 - dist_f1
            print(f"  {label:<15}: {rob_f1:.3f} vs {dist_f1:.3f} ({diff:+.3f})")
        
        # Overall recommendation
        micro_improvement = roberta_results['micro_f1'] - distilbert_micro
        macro_improvement = roberta_results['macro_f1'] - distilbert_macro
        
        if micro_improvement > 0.02 or macro_improvement > 0.05:
            print(f"\n✅ Recommendation: RoBERTa shows meaningful improvement!")
        elif micro_improvement > -0.01 and macro_improvement > -0.02:
            print(f"\n🤷‍♂️ Recommendation: RoBERTa similar to DistilBERT")
        else:
            print(f"\n✅ Recommendation: DistilBERT performs better")
    
    else:
        print("❌ No DistilBERT results found for comparison!")

def main():
    """Main training and evaluation pipeline"""
    print("🤖 RoBERTa Issue Classifier Training")
    print("=" * 50)
    ...
    try:
        ...
        compare_with_distilbert(results)
